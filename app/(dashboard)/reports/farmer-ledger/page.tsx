  ...
  function printSummary() {
    ...
    const rows = filtered.map((f, i) => {
      ...
      return `<tr style="${i % 2 === 0 ? "background:#f9fdf9" : ""}">
        <td>${i + 1}</td><td><strong>${f.name}</strong></td><td>${f.village || "—"}</td><td>${f.phone || "—"}</td>
    ...
    w.document.write(`<html><head><title>All Farmers Report</title><style>${reportCSS} body{max-width:900px;margin:0 auto}</style></head><body>
${buildPrintHeader(shop)}
<div class="doc-header"><div><div class="doc-title">All Farmers Report</div><div class="doc-sub">${filtered.length} farmers</div></div><div class="doc-meta"><div>Printed: ${date}</div></div></div>
<div class="body-pad"><table>
<thead><tr><th>#</th><th>Name</th><th>Village</th><th>Phone</th><th style="text-align:right">Total Dr</th><th style="text-align:right">Total Cr</th><th style="text-align:right">Balance</th><th style="text-align:center">Status</th></tr></thead>
<tbody>${rows}</tbody>
<tfoot><tr><td colspan="4"><strong>${filtered.length} farmers</strong></td>
...
<td style="text-align:right;color:#b91c1c"><strong>PKR ${totalPayable.toLocaleString()}</strong></td><td></td></tr></tfoot>
